...
class PerceptronVisualizer(BaseVisualizer):
    ...
    def __init__(self, save_dir: Union[str, Path] = "outputs/plots", enabled: bool = True):
        ...
        logger.info(f"PerceptronVisualizer initialized (enabled: {enabled})")
    
    def plot_confusion_matrix(self, y_true: np.ndarray, y_pred: np.ndarray,
                             class_names: Optional[List[str]] = None,
                             title: str = "Perceptron Confusion Matrix",
                             save_name: str = "confusion_matrix") -> Optional[Figure]:
...
• Misclassifications: May indicate non-linear
  separability or insufficient training"""
        ...
        insights = self._generate_performance_insights(accuracy, precision, recall)
        add_performance_insights(fig, insights)
    ...
    def plot_learning_curve(self, errors_per_epoch: List[int],
                          ...
                          save_name: str = "learning_curve") -> Optional[Figure]:
        ...
        if len(errors_per_epoch) > 0 and errors_per_epoch[-1] == 0:
            ...
            ax_main.axvline(x=convergence_epoch, color=self.colors['success'],
...
{'• Data appears linearly separable' if final_errors == 0 else '• Check data separability'}"""
    ...
    def plot_decision_boundary(self, model, X: np.ndarray, y: np.ndarray,
                             ...
                             save_name: str = "decision_boundary") -> Optional[Figure]:
        ...
        ax_main.contourf(xx, yy, Z, alpha=0.4, colors=colors_boundary, levels=1)
        
        # Plot data points with enhanced styling
        unique_labels = np.unique(y)
        for i, label in enumerate(unique_labels):
            mask = (y == label)
            ax_main.scatter(X[mask, 0], X[mask, 1], 
        ...
        try:
            ...
            logger.debug(f"Could not draw analytical decision boundary: {e}")
...
• Historically significant (1957)"""
    ...
    def generate_all_visualizations(self, model, X: np.ndarray, y: np.ndarray,
        ...
        figures['confusion_matrix'] = self.plot_confusion_matrix(
            y, y_pred, class_names, save_name="confusion_matrix"
        )
        ...
        if X.shape[1] == 2:
            figures['decision_boundary'] = self.plot_decision_boundary(
                model, X, y, class_names, save_name="decision_boundary"
            )
    ...
    def _generate_performance_insights(self, accuracy: float, 
        ...
        if accuracy >= 0.95:
            level = "Excellent ⭐⭐⭐"
            message = "Perfect or near-perfect classification! The perceptron has successfully learned the decision boundary."
        elif accuracy >= 0.85:
            level = "Good ⭐⭐"
            message = "Strong performance with room for improvement. Consider feature engineering or more training."
        elif accuracy >= 0.70:
            level = "Fair ⭐"
            message = "Moderate performance. Data may not be linearly separable or needs preprocessing."
        else:
            level = "Poor"
            message = "Low performance suggests data is not linearly separable. Consider non-linear approaches."
        
        return f"Performance Level: {level}\n{message}"


# Backward compatibility functions for existing code
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, 
                         class_names: Optional[List[str]] = None) -> Figure:
    """Backward compatibility wrapper for confusion matrix."""
    visualizer = PerceptronVisualizer(enabled=True)
    return visualizer.plot_confusion_matrix(y_true, y_pred, class_names, save_name=None)


def plot_learning_curve(errors_per_epoch: List[int]) -> Figure:
    """Backward compatibility wrapper for learning curve."""
    visualizer = PerceptronVisualizer(enabled=True)
    return visualizer.plot_learning_curve(errors_per_epoch, save_name=None)


def plot_decision_boundary(model, X: np.ndarray, y: np.ndarray, 
                          class_names: Optional[List[str]] = None) -> Optional[Figure]:
    """Backward compatibility wrapper for decision boundary."""
    visualizer = PerceptronVisualizer(enabled=True)
    return visualizer.plot_decision_boundary(model, X, y, class_names, save_name=None)


# Legacy wrapper class for complete backward compatibility
class Visualizer:
    """Legacy visualizer class for backward compatibility."""
    
    def __init__(self):
        self._visualizer = PerceptronVisualizer(enabled=True)
        logger.info("Legacy Visualizer initialized (using shared framework)")
    
    def plot_confusion_matrix(self, y_true: np.ndarray, y_pred: np.ndarray,
                             class_names: Optional[List[str]] = None) -> Figure:
        return self._visualizer.plot_confusion_matrix(y_true, y_pred, class_names, save_name=None)
    
    def plot_learning_curve(self, errors_per_epoch: List[int]) -> Figure:
        return self._visualizer.plot_learning_curve(errors_per_epoch, save_name=None)
    
    def plot_decision_boundary(self, model, X: np.ndarray, y: np.ndarray,
                              class_names: Optional[List[str]] = None) -> Optional[Figure]:
        return self._visualizer.plot_decision_boundary(model, X, y, class_names, save_name=None)
